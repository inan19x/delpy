...
delpy monitors a configured directory using the Python watchdog library. When a file event occurs, delpy reads the file content and checks it against the patterns defined in signatures/delpy-pattern.txt. Each pattern has a name, regular expression, and sensitivity level, allowing the learners to see how content-based detection works.

## Execution instruction:
Console#1 execute: 
> python3 src/delpy.py

Console#2 execute:
> echo "Test card 4111 1111 1111 1111" > /tmp/test-card.txt
